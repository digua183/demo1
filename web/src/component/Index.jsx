...
function IndexItem({ data: { time, picture, nickname, urgent, releaseType, otherRemarks, img }, ...props }) {
    return <div className="item"  {...props}>
        <div className="release-time">{time ? `${new Date(time).toLocaleDateString()}  ${new Date(time).toLocaleTimeString()}` : null}</div>
...
class Index extends React.Component {
    ...
    deleteType = (e, value) => {
        ...
        this.setState({ items: items.filter(t => t != value) })
    }
    ...
    releaseContent = () => {
        let { releaseType, urgent, dataSource, previewVisible, previewImage, fileList, items, name, showIcon } = this.state;
        const uploadButton = (
            <div>
                <PlusOutlined />
        ...
        return <div className="content">
            <Radio.Group defaultValue={releaseType} style={{ marginBottom: 30 }} onChange={this.handleTypeChange}>
            ...
            <div>
                ...
                >
                    <Form.Item label="物品类型：" name="type">
                        ...
                        >
                            {items.map(item => (
                                <Option key={item}>{item}{showIcon ? <DeleteOutlined onClick={(e) => this.deleteType(e, item)} style={{ float: "right" }} /> : null}</Option>
                            ))}
                        </Select>
                    </Form.Item>
                    ...
                    <Form.Item label="联系方式：" name="contactMethod">
                        <Input placeholder="QQ/微信/手机号/其他" />
                    </Form.Item>
                    ...
                    <Form.Item label="正文：" name="otherRemarks">
                        <Input.TextArea
                            ...
                            placeholder={releaseType == "拾物" ? "如果物品中内容较多，可简要列出物品的内容名称，以便失主核对是否为本人丢失，注意证件和饭卡的号码、人民币的数目均不能详写，以防他人冒领" :
